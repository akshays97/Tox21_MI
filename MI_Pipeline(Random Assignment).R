...
setwd("/ddn/gs1/home/sankara2/Data/Pipetest4")
...
foreach(k= 1:30) %dopar% {
  data.sub[,17:31]= t(apply(data.sub[,17:31], MARGIN = 1, sample))
...
info <- function(arg1, arg2, data, p){
  data.mat = data
  ...
  if (p){
    post=  c(pos1, pos2)
    #print(post)
    pos1 <- sample(post, size = length(pos1))
    #print(pos1)
    pos2 <- setdiff(post, pos1)
    #print(pos2)
    #print("NEW LINE")
  }
...
foreach(i= 0:30) %dopar% {
  ...
  for (a in 1: length(pwarray[1,])){
    ...
    dat[a,] = try(info(t1[a], t2[a], data.mat, p = z) )
  }
